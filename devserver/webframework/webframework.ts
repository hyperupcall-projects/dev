...
export function startServer(args: string[]) {
	...
	createApp(app, wss)
	app.get('/lib.ts', (req, res) => serveJs(req, res, './devserver/webframework') as any)
	...
	res.send(
		dedent(`<!doctype html>
							<html>
								...
								<script type="module">
								globalThis.ws = new WebSocket("http://localhost:3000/ws"); // TODO: hardcoded port
								globalThis.ws.addEventListener("error", console.error);
								globalThis.ws.addEventListener("open", () => {
